...
public class inventario : MonoBehaviour
{
    ...
    private class Arma5
    {
        public TodasArmas TipoArma { get; set; }
    ...
    void OnGUI()
    {
        ...
        /* foreach(int i in tipoArma){
             foreach(int y in countItens){
        ...
        if (tipoArma[0] == "Lamina" || tipoArma[1] == "Lamina" || tipoArma[2] == "Lamina" || tipoArma[3] == "Lamina"
         || tipoArma[4] == "Lamina" || tipoArma[5] == "Lamina" || tipoArma[6] == "Lamina" || tipoArma[7] == "Lamina"
         || tipoArma[8] == "Lamina" || tipoArma[9] == "Lamina")
        {
        ...
        while (count < 3)
        {
            ...
            if (GUI.Button(new Rect(posicaoX, posicaoY, largura, altura), texturaItens[count]))
            {
                ...
                if (fazTroca && indiceCliqueFinal != -1)
                {
    ...
    void Update()
    {
        ...
        if (Input.GetKeyDown("e") && AbrirTorre.podeMovimentar)
        {
            Destroy(ItenAtual);
        ...
        if (tipoArma[changeGun - 1] == "Lamina")
        {
            if (Input.GetMouseButtonDown(0))
            {
                GetComponent<Animation>().Play("Atckmelee01");
            }
        }
    ...
    void Create_Gun()
    {
        IList armas = new List<Arma>();
       // armas.Add(new Arma { TipoArma = TodasArmas.Lamina, HasFound = false });


        IDictionary arma = new Dictionary<TodasArmas, bool>();
        arma.Add(TodasArmas.LAMINA, false);
    }
}